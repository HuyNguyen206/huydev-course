...
<div>
    ...
    <h2 v-if="current_plan">Your current plan: <label for="" class="badge badge-success"> {{ current_plan.product.name }}</label></h2>
    <h2 v-else>You haven't subscribe yet</h2>
    ...
    <button v-if="current_plan" style="margin-top: 30px;" :disabled="loading" class="btn btn-primary" @click="updatePlan">
...
export default {
    ...
    data(){
      return {
          ...
          plan_id:JSON.parse(this.current_plan_json).id,
          loading:false,
          error: ''
      }
    },
